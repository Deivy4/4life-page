...
export default function Product({ urlProducto, id: idProduct, name, description, urlImage } : Product4life){
    ...
    const handleClick = async ()=>{
        ...
        if(isArgentina()){
            let productWithPrice : Product4life = await GetProductWithPrice({ idProduct : idProduct ?? 0 });
            openSideBar()
    ...
    return (
        <div className={`${isOpen ? " opacity-50" : "opacity-100"} group text-blue-400  cursor-pointer w-[280px] rounded p-2 py-4 hover:bg-blue-700 hover:bg-opacity-40`}>
            ...
            <h2 className="mt-3 text-2xl text-center text-blue-800 font-bold">{name}</h2>
